...
pub trait SecretSharer {
    ...
    fn prime(&self) -> Result<&BigInt, SSError>;
    /// Split a secret to shares based on SecretSharing params.
    fn generate_shares(&mut self, secret: &str) -> Result<Vec<String>, SSError> {
    ...
    fn reconstruct_secret(&self, shares: &[String]) -> Result<String, SSError> {
...
pub struct PlaintextSecretSharer {
...
impl PlaintextSecretSharer {
    pub fn new(threshold: u32, total: u32) -> Self {
        PlaintextSecretSharer {
            threshold,
            total,
            prime: None,
        }
...
impl SecretSharer for PlaintextSecretSharer {
    ...
    #[inline]
    fn charset(&self) -> &str {
        Charset::Alphanumeric.charset_str()
    }
